...
import PlateformPickermodel from "../components/PlateformPickermodel"
...
const Accounts = () => {
    ...
    const handleConnect = async (plarformId:string) => {
        setConnecting(plarformId)
        setTimeout(() => {
            setConnecting(null)
    ...
    <div className="space-y-8 max-w-4xl">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-sm">
            <div>
                ...
                <p className="text-slate-500 text-sm mt-0.5">{accounts.length} of {PLATFORMS.length} platform connected</p>
            </div>
        ...
        {/* platofrom picker model */}
        {showPlatformPicker && <PlateformPickermodel connectedIds={connectedIds} connecting={connecting} onClose={()=> setShowPlatformPicker(false)} onConnect={handleConnect}/>}
