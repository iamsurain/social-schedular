...
const AIComposer = () => {
  ...
  const [tone, setTone] = useState("Professinal");
  const [generateImage, setGenerateImage] = useState(true);
  const [loading, setLoading] = useState(false);
  const [generations, setGenerations] = useState<any[]>([]);

  // scheduling state
  const [activeSchedular, setaActiveSchedular] = useState<any>(null);
        ...
        <div className="relative group mt-12">
          ...
          <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
            ...
            >
              ...
              >
                <span
                  className={`pointer-events-none size-4 transform rounded-full translate-y-0.5 bg-white transition ${generateImage ? "translate-x-4.5" : "translate-x-0.5"}`}
                />
              </div>
            </button>
            ...
            >
              <div className="flex flex-col h-full space-y-4">
                ...
                <div className="flex items-center gap-2 pt-2">
                  <button
                    onClick={() => setaActiveSchedular(gen)}
        ...
        <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-8 py-4 border-b border-slate-100 bg-slate-50/30">
              ...
              <button
                onClick={() => setaActiveSchedular(null)}
            ...
            <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {PLATFORMS.map((p)=>{
                    ...
                    return (
                      <button key={p.id} onClick={()=> setSelectedPlatforms((prev)=> (prev.includes(p.id) ? prev.filter((x)=>x !== p.id) : [...prev, p.id]))} className={`p-2.5 rounded-md border text-xs ${active ? "bg-red-500 text-white border-red-500" : "bg-white border-slate-200 text-slate-400 hover:border-slate-300"}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="relative">
                    ...
                    <input type="date" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all" value={scheduledDate} onChange={(e)=> setScheduledDate(e.target.value)} />
                  </div>

                  <div className="relative">
                    <ClockIcon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/>
                    <input type="time" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all" value={scheduledTime} onChange={(e)=> setScheduledTime(e.target.value)} />
                  </div>
              </div>
            </div>
            <button onClick={handlSchedule} className="w-full flex items-center justify-center gap-2 py-3 rounded-md bg-slate-200 text-slate-700 hover:bg-red-500 hover:text-white transition">
