...
const Dashboard = () => {
    ...
    useEffect(()=>{
        const fetchDashobardData = async () =>{
            try {
                const [postRes, acoountsRes, activityRes] = [{data: dummyPostsData}, {data: dummyAccountsData}, {data: dummyActivityData}]
                const posts = postRes.data;
                setStats({
                    scheduled: posts.filter((p: any) => p.status === 'scheduled').length,
                    published: posts.filter((p: any) => p.status === 'published').length,
                    connectedAccounts: acoountsRes.data.filter((a: any) => a.status === 'connected').length,
                })
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-slate-100">
        ...
        {activities.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 px-6">
                <div className="size-12 bg-slate-100 rounded-xl flex items-center justify-center mb-3">
                    ...
                    <p className="text-slate-500">No activty yet</p>
